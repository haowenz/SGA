...
class SequenceGraph {
 public:
  ...
  void GenerateCharLabeledGraph() {
    ...
    GraphSizeType vertex_id =
        compacted_graph_labels_
            .size();  // Keep the original vertex ids unchanged.
    ...
    for (std::string &compacted_graph_label : compacted_graph_labels_) {
      ...
      if (compacted_graph_label_length == 1) {
        adjacency_list_[compacted_graph_vertex_id].insert(
            ...
            compacted_graph_adjacency_list_[compacted_graph_vertex_id]
                .end());  // add the neighbors of the chain to the neighbors of
                          // the last vertex
      }
      for (GraphSizeType i = 1; i < compacted_graph_label_length; ++i) {
        ...
        if (i == 1) {  // the second vertex in the chain
          adjacency_list_[compacted_graph_vertex_id].push_back(
              vertex_id);  // add the link from the first vertex to the second
                           // vertex in the chain
        }
        if (i + 1 < compacted_graph_label_length) {  // not the last vertex in
                                                     // the chain, which means
                                                     // it has a next vertex
          adjacency_list_[vertex_id].push_back(
              vertex_id + 1);  // add link from current vertex to its next
          ...
          adjacency_list_[vertex_id].insert(
              ...
              compacted_graph_adjacency_list_[compacted_graph_vertex_id]
                  .begin(),
              compacted_graph_adjacency_list_[compacted_graph_vertex_id]
                  .end());  // add the neighbors of the chain to the neighbors
                            // of the last vertex
        }
        ++vertex_id;
      }
  ...
  ScoreType AlignUsingLinearGapPenalty(const sga::Sequence &sequence) {
    ...
    ScoreType reverse_complement_alignment_cost =
        current_layer[current_order[0]];
    ScoreType min_alignment_cost =
  ...
  QueryLengthType AlignUsingLinearGapPenaltyWithNavarroAlgorithm(
    ...
    QueryLengthType reverse_complement_alignment_cost =
        *std::min_element(current_layer.begin(), current_layer.end());
    QueryLengthType min_alignment_cost =
        std::min(forward_alignment_cost, reverse_complement_alignment_cost);
    std::cerr << "Sequence length: " << sequence_length
              << ", forward alignment cost:" << forward_alignment_cost
              << ", reverse complement alignment cost:"
