...
class SequenceBatch {
 public:
  ...
  uint32_t LoadBatch() {
    ...
    for (uint32_t sequence_index = 0; sequence_index < max_batch_size_; ++sequence_index) { 
      ...
      if (length == 0) 
        ...
        if (sequence_kseq_->qual.l == 0) { // fasta file
          sequence_batch_[sequence_index].Update(length, sequence_kseq_->name.s, sequence_kseq_->seq.s);
        } else { // fastq file
          sequence_batch_[sequence_index].Update(length, sequence_kseq_->name.s, sequence_kseq_->seq.s, sequence_kseq_->qual.s);
        }
        ++num_sequences;
      } else {
        assert(length == -1); // make sure to reach the end of file rather than meet an error
        break;
      }
    }
    std::cerr << "Number of sequences: " << num_sequences << "." << std::endl;
    std::cerr << "Loaded sequence batch successfully in " << sga::GetRealTime() - real_start_time << "s." << std::endl;
