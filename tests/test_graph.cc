...
TEST_F(SequenceGraphTest, LoadFromTxtFileTest) {
  ...
  EXPECT_EQ(num_vertices, (uint32_t)2320) << "Number of vertices loaded is wrong! It should be 2320 but it is " << num_vertices;
  uint32_t num_edges = txt_sequence_graph_.GetNumEdgesInCompactedGraph();
  EXPECT_EQ(num_edges, (uint32_t)2320) << "Number of edges loaded is wrong! It should be 2319 but it is " << num_edges;
}

TEST_F(SequenceGraphTest, GenerateCharLabeledGraphTest) {
  uint32_t num_vertices = txt_sequence_graph_.GetNumVertices();
  EXPECT_EQ(num_vertices, (uint32_t)139189) << "Number of vertices loaded is wrong! It should be 139189 but it is " << num_vertices;
  uint32_t num_edges = txt_sequence_graph_.GetNumEdges();
  EXPECT_EQ(num_edges, (uint32_t)(139189)) << "Number of edges loaded is wrong! It should be 139188 but it is " << num_edges;
}
...
TEST_F(SequenceGraphTest, AlignUsingLinearGapPenaltyTest) {
  ...
  for (uint32_t i = 0; i < num_loaded_sequences; ++i) {
    int32_t alignment_score = txt_sequence_graph_.AlignUsingLinearGapPenalty(sequence_batch_.GetSequence(i));
    EXPECT_EQ(alignment_score, max_alignment_scores[i]) << "Alignment score for sequence" << i << " is wrong! It should be " << max_alignment_scores[i] << " but it is " << alignment_score;
  }
}
...
TEST_F(SequenceGraphTest, AlignUsingLinearGapPenaltyWithNavarroAlgorithmTest) {
  ...
  txt_sequence_graph_.SetAlignmentParameters(1, 1, 1);
  for (uint32_t i = 0; i < num_loaded_sequences; ++i) {
    int32_t alignment_score = txt_sequence_graph_.AlignUsingLinearGapPenaltyWithNavarroAlgorithm(sequence_batch_.GetSequence(i));
    EXPECT_EQ(alignment_score, max_alignment_scores[i]) << "Alignment score for sequence" << i << " is wrong! It should be " << max_alignment_scores[i] << " but it is " << alignment_score;
  }
}
}  // namespace sga_testing
